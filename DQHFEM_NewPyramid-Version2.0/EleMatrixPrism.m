function  [K,M] =EleMatrixPrism(ElementData,E,mum,rhom)
...
IP=IntegralPntPrism(nq);
%% 求取积分点处的形函数值
[G,Gr,Gs,Gt]=BasePrism(IP.Ri,IP.Si,IP.Ti,number);
%% 将边界上的形函数转化为插值形式
T=InterpPrism(number);
[GG,GR,GS,GT]=InterpTrans(G,Gr,Gs,Gt,T);
%% 将形函数关于自然坐标r,s,t的导数值映射为关于真实坐标x,y,z的导数值
x=eleX{1};y=eleX{2};z=eleX{3};
[~,~,~,dX,dY,dZ,dJ]=MapCurPrism(IP.Ri,IP.Si,IP.Ti,x,y,z,MapNumber);
